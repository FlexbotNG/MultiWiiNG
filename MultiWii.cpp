...
void annexCode() { // this code is excetuted at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  for(axis=0;axis<3;axis++) {
    ...
    if(axis!=2) { //ROLL & PITCH
        ...
        if (f.ANGLE_MODE)
        {
          // QUADX机型在Angle模式下，D/R小舵设置为：副翼 60%，升降 60%。Skypup 2015.04.01
          tmp = tmp * 6 / 10;
        }
      #endif
      ...
      dynD8[axis] = (uint16_t)conf.pid[axis].D8*prop1>>7; // was /100, is /128 now
        ...
        if (f.ANGLE_MODE)
        {
          // QUADX机型在Angle模式下，D/R小舵设置为：方向 70%。Skypup 2015.04.01
          tmp = tmp * 7 / 10;
        }
      #endif
      rcCommand[axis] = tmp;
  ...
  if ((int16_t)(currentTime-rcTime) >0 ) { // 50Hz
    ...
    if(rcDelayCommand == 20) {
      if(f.ARMED) {                   // actions during armed
        ...
        //        #if ACC
        //          // 禁用！防止全油门拉杆时，误打死方向舵进入水平校准。
        //          else if (rcSticks == THR_HI + YAW_LO + PIT_LO + ROL_CE) calibratingA=512;     // throttle=max, yaw=left, pitch=min
        //        #endif
        //        #if MAG
        //          // 禁用！防止全油门拉杆时，误打死方向舵进入地磁校准。
...
//        #endif    
  ...
  */
  if (f.BARO_MODE) {
    ...
    #ifdef ALT_HOLD_THROTTLE_MIDPOINT
      if (lLockBaroTakeoff) // 定高模式起飞，当油门未超过一半时，油门输出最小值。Skypup 2015.04.30
      {
  ...
  #else
    if (f.ANGLE_MODE || f.HORIZON_MODE)  rcCommand[THROTTLE] += BaroPID_D;
  #endif //BARO
